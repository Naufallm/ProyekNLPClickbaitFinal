...
def inject_custom_css_app():
    ...
    <style>
        ...
        h1#prediksi-data-baru, /* ID BARU untuk halaman prediksi gabungan */
        h1#ringkasan-kinerja { /* ID yang sudah ada */
            color: #00BFFF;
...
navigation_options_app = {
    ...
    "🔮 Prediksi": "Prediksi" # <--- MENU PREDIKSI DIGABUNGKAN
}
selected_page_key_app = st.sidebar.radio(
    "Pilih Halaman:", list(navigation_options_app.keys()), key="sidebar_nav_app_v5" # Key diubah untuk update
)
...
if selected_page_app == "Ringkasan Kinerja":
    ...
    selected_model_display_app = st.selectbox("Pilih Model:", available_model_display_names_app, key="detail_model_select_app_styled_v2")
    ...
    selected_model_display_err_app = st.selectbox("Pilih Model:", available_model_display_names_app, key="error_model_select_app_styled_v2")
...
elif selected_page_app == "Prediksi": # <--- KODE UNTUK HALAMAN PREDIKSI YANG DIGABUNGKAN
    st.markdown("<h1 id='prediksi-data-baru'>🔮 Prediksi Data Baru</h1>", unsafe_allow_html=True)
    st.markdown("---")

    prediction_mode_app = st.radio(
        "Pilih Mode Prediksi:",
        ("Prediksi Teks Tunggal", "Prediksi dari File CSV"),
        key="pred_mode_select_app_v1", # Key baru
        horizontal=True # Tampilkan radio button secara horizontal
    )
    st.markdown("---") # Pemisah tambahan

    if prediction_mode_app == "Prediksi Teks Tunggal":
        st.subheader("🖋️ Prediksi Teks Tunggal (Judul Berita)")
        st.markdown("Uji coba model klasifikasi dengan memasukkan judul berita baru di bawah ini.")
        selected_model_display_pred_app = st.selectbox(
            "Pilih Model untuk Prediksi:",
            available_model_display_names_app,
            key="live_pred_model_select_app_styled_v3", # Key diperbarui
            help="Model yang dipilih akan digunakan untuk prediksi."
        )
        news_input_app = st.text_area(
            "Masukkan Judul Berita:",
            height=150,
            key="live_pred_input_app_styled_v3", # Key diperbarui
            placeholder="Contoh: HEBOH! Penemuan Spesies Baru di Dasar Laut Terdalam Mengejutkan Ilmuwan!"
        )
        if st.button("✨ Prediksi Sekarang!", key="live_pred_button_app_styled_v3"): # Key diperbarui
    ...
    elif prediction_mode_app == "Prediksi dari File CSV":
        st.subheader("📄 Prediksi dari File CSV Unggahan")
        ...
        uploaded_file_app = st.file_uploader(
            "Pilih file CSV (maks. 200MB)",
            type=["csv"],
            key="csv_uploader_app_v3", # Key diperbarui
            help="File CSV dengan satu kolom berisi teks judul berita."
        )
        ...
        if uploaded_file_app is not None:
            try:
                ...
                selected_text_column_app = st.selectbox(
                    ...
                    index=0,
                    key="csv_text_column_select_app_v3", # Key diperbarui
                ...
                selected_model_display_csv_app = st.selectbox(
                    ...
                    available_model_display_names_app,
                    key="csv_analysis_model_select_app_v3", # Key diperbarui
                    help="Model yang akan digunakan untuk mengklasifikasikan teks dari file CSV."
                )

                if st.button("🚀 Proses dan Prediksi CSV!", key="process_csv_button_app_v3") and                    selected_text_column_app and                    selected_model_display_csv_app and                    selected_model_display_csv_app != "Data model tidak tersedia":
                            ...
                            if not classifier_model_app:
                                ...
                                if features_for_prediction_app is not None:
                                    ...
                                    st.download_button(
                                        ...
                                        key="download_csv_pred_app_v3" # Key diperbarui
                                    )
